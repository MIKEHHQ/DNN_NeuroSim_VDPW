...
parser.add_argument('--model', default='MobileNetV1', help='VGG8|DenseNet40|ResNet18|ResNet20|MobileNetV1|MobileNetV2|DSnet')
...
if args.model == 'VGG8':
    ...
    from models import resnet20_official_modified
    # model_path = './log/Resnet20_model_best.pth.tar'
    # model_path = './log/Resnet20_model_best.pth.tar'
    model_path = './log/Resnet20_92.54.pth.tar'
    net_name = args.model
    modelCF = resnet20_official_modified.resnet20_cifar(num_classes=10, args = args, logger=logger)
    state_dict = torch.load(model_path)["state_dict"]
    new_state_dict = {}
    for k,v in state_dict.items():
        new_state_dict[k[7:]] = v
    modelCF.load_state_dict(new_state_dict)
    total_num_params = sum([p.numel() for p in modelCF.parameters()])
    trainable_num_params = sum(p.numel() for p in modelCF.parameters() if p.requires_grad)
    ...
    # model_path = '/home/zyguan/Inference_pytorch/DS_1/ADCprecision=5/batch_size=128/cellBit=4/dataset=cifar10/decreasing_lr=140,180/detect=0/grad_scale=8/inference=0/lr=0.01/mode=FP/model=DSnet/onoffratio=10/seed=117/subArray=128/t=0/target=0/v=0/vari=0/wl_activate=8/wl_error=8/wl_grad=8/wl_weight=8/best-39.pth'
    model_path = './pretrained_models/DSC_1layer.pth'
    ...
    net_name = args.model
    modelCF = mobilenetv1.MobileNetV1(num_classes=10, args=args, logger=logger,pretrained=model_path)
    print(modelCF)

    # summary(modelCF,input_size =(3,16,16))
elif args.model == 'MobileNetV2':
    from models import mobilenetv2
    model_path = '/pretrained_models/mbv1_pr'
    # model_path = './log/mbv1.pth'
    net_name = args.model
    modelCF = mobilenetv2.MobileNetV2(num_classes=10, args=args, logger=logger)
    print(modelCF)
else:
