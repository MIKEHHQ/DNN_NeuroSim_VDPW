...
class MobileNet(nn.Module):
    ...
    def __init__(self, class_num=10, args=None, logger=None):
        ...
        self.mobilebone = nn.Sequential(
            # DepthSeperabelConv2d(32, 128, 1,args=args, logger=logger),
            # DepthSeperabelConv2d(32, 64, 1, args=args, logger=logger),
            DepthSeperabelConv2d(3, 64, 1, args=args, logger=logger),
        ...
        # self.conv2 = Conv2d(512,512,3, stride=1,padding=1,args=args, logger=logger)
        # self.conv3 = Conv2d(512,512,3, stride=1, padding=1, args=args, logger=logger)
        self.avg = nn.AdaptiveAvgPool2d(1)
        # self.avg = nn.MaxPool2d(kernel_size=4,stride=4)
        # self.fc1 = Linear(8192, 1024, args=args, logger=logger)
        self.fc = Linear(512, class_num, args=args, logger=logger)
    def forward(self, x):
        # x = self.conv1(x)
